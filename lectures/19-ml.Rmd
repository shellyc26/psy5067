...
.small[Note, newer package called `tidymodels` is better for machine learning, but requires a lot more steps. For now, this is the simpler method. Check out `tidymodels` if you're going hard with ML, check out `tidymodels` or use Python `r emo::ji("snake")`]
```{r, message = F}
library(caret)
...
The remaining lectures will be on repeated measures ANOVA and hierarchical linear modeling (HLM). They will be given by our very own Violet Brown!

Violet is going for her teaching certificate. After her lectures, we will send out a quick survey -- please, please, please fill this out! 
