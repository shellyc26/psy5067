...
Semi-partial: used when we want to show that some variable adds incremental variance in Y above and beyond another X variable. How much unique variance an IV adds to the TOTAL amount of variance.

- e.g., predicting Alzheimer's

Partial: used when some third variable, Z, is a plausible explanation of the correlation between X and Y. How much unique variance adds to the unaccounted variance in Y.sq 
...
### $e$ are part of $Y$ that is independent of $X$

$$ \hat{Y} = b_0 + b_1X$$
$$ e_i = Y_i - \hat{Y_i}$$
We can use this to construct a measure of $X_1$ that is independent of $X_2$:

$$ \hat{X}_{1.2} = b_0 + b_1X_2$$

$$e_{X_1} = X_1 - \hat{X}_{1.2}$$
.small[
