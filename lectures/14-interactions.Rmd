...
Consider the case where *D* is a *d*ummy coded variable representing two groups. In a univariate regression, how do we interpret the coefficient for D?
...
This is called "ANCOVA". It's just regression.
...
Just like we use $R^2$ to report how much variance in Y is explained by the model, we can look at the contributions of each variable in the model, including factors with 3+ levels. 
...
Now extend this to include joint effects:
...
Wash U contacts 150 alumni and collects their current salary (in thousands of dollars), their primary undergraduate major, and their GPA upon graduating. 
...
```{r, message = F, warning = F, fig.width=10, fig.height=6, eval = FALSE}
...
```{r, message = F, warning = F, fig.width=10, fig.height=6, echo=FALSE}
...
3 Hypothesies. 2 correspond to questions that would arise in a simple one-way ANOVA:
...
The ANOVA will be able to tell us if the means are significantly different  and the magnitude of those differences in terms of variance accounted for.
