...
- Re-write equation to include the means of Y and X (xbar & ybar)
...
- Notice that when  $X = \bar{X}$ the regression line goes through  $\bar{Y}$ . This is true for all regressions such that the regression line must bass through $\bar{X}$  and  $\bar{Y}$
...
- Residuals are not correlated with $X$ and $\hat{Y}$ because those two are perfectly correlated with one another (that is, $r(\text{fitted and X}) = 1$  )
