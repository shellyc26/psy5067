...
---
.left-column[
It doesn't make sense! Predicted values can go above 1 or below 0...yikes
]
```{r out.width = '75%', echo = FALSE}
...
Two most popular are logit and log.
]
...
Taking the log of the odds solves this problem by making everything symmetrical!
...
For each 1-unit change in X, holding other predictors constant, the odds of the outcome occurring (that Y = 1) changes by $e^{b_{1}}$

```{r}
exp(1)^.08012
```

*For every 1-unit increase in Intelligence, the odds of not having a premature death increase by ~8%*
