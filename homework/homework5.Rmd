...
1. Using the `glm` function, run a multiple regression with IQ to predict and mortality status. Past studies have found that higher levels of IQ are associated with longer life. Use this information to make sure you interpret the direction of your results correctly. Then run the same function using the `lm` function. What are the similarities, what are the differences between these two approaches? Look at both the `anova` and the `summary` outputs. 

2. Now control for risky behavior. Interpret all of the coefficients. 

3. Instead of using a logit link function, rerun the analysis in #2 using a probit link function. What differences do you notice? 
