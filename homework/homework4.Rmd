...
5. Try the `check_model()` function from the `performance` package for your initial model. Does it meet the assumptions?

6. These data are set up perfectly to run a 2x2 ANOVA:

  - Use the model comparison approach to test the hypothesis that there is a main effect of Therapy on Anxiety scores. Then use the `aov` function to check your work. 
  - Use the model comparison approach to test the hypothesis that there is a main effect of Sleep on Anxiety scores. Use `aov` to check your work.
  - Use the model comparison approach to test the hypothesis that there is an interaction of Sleep and Therapy on Anxiety scores. Use `aov` to check your work. 

7. We have talked about interactions in many different forms during this unit! Pick a variant that you feel the least comfortable with (2 continuous variables, 2 categorical variables, or 1 of each, polynomial, 3way interaction etc.). Ask Chat-GPT (or any AI tool) to generate a small dataset for you such that you can practice. Use the dataset it provides to:

- Run the model (if you want more practice with model comparisons, that's OK too)
_ Write the equation
- Interpret all coefficients
- Interpret the omnibus test
- Create an appropriate visualization of the interaction
